...
dag = DAG(
    dag_id='offices_over_3_points_310',
    default_args=default_args,
    schedule_interval='22 6 * * *',
    description='Заказы с марштрутом более 3 офисов.',
    catchup=False,
    max_active_runs=1,
...
#если мы в отчете показываем заказы с последним dt_load, то тогда надо брать инфу из таблицы за последние 5 дней,
#обновляем 1 раз в день, и если я правильно понимаю тогда будут дубли, нужно дргой движок у витрины сделать?
...
def main():
    ...
    for i in range(0, len(quantile_result) - 1):
        ...
        table1_query = f"""                    
                   ...
                   from history.order_completed
                   where dt >= now() - interval 5 day - interval 8 hour
                       and rid_hash between {rid_start} and {rid_end}
                       and not in (select rid_hash from {dst_table})
                   group by rid_hash
               """
...
task1 = PythonOperator(
    task_id='offices_over_3_points_310', python_callable=main, dag=dag)
